...
1. Bump `version` in `package.json` (update a `CHANGELOG.md` if one exists).
...
4. `git tag vX.Y.Z && git push origin vX.Y.Z`; `gh release create vX.Y.Z`.
5. Update the `dasasian.com/firebase-structured-logger` page in `dasasian-web`. Only
